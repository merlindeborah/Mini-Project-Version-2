...
def visualize_graph(graph, title="Graph"):
    ...
    nx.draw(graph, pos, with_labels=True, node_color='lightgrey', node_size=500)
    plt.title(title)
    plt.show()

# Function to visualize the communities
def visualize_communities(graph, communities, title="Communities"):
...
def form_initial_communities(graph, importance):
    ...
    visualize_communities(graph, communities, title="Initial Communities")  # Visualize initial communities
    return communities
...
def assign_overlapping_nodes(graph, communities):
    ...
    visualize_communities(graph, final_communities, title="Communities After Resolving Overlaps")  # Visualize after resolving overlaps
...
def merge_communities(graph, communities, mc=1.5):
    ...
    visualize_communities(graph, final_communities, title="Final Merged Communities")  # Visualize final communities
    return final_communities
...
if __name__ == "__main__":
    # Load the graph from the .mtx file
    file_path = 'karate.mtx'  # Update this path if needed
    ...
    if validate_communities(G, communities):
        ...
        visualize_communities(G, communities, title="Final Communities")
    else:
